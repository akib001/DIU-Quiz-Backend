...
exports.userSignup = async (req, res, next) => {
    ...
    try {
        ...
        if (user) {
            if (user.role == 'admin') {
                const error = new Error('You are already admin. Please login with your old credentials');
        ...
        const newUser = new User({
            email: email, password: hashedpw, name: name, role: role,
        });
...
exports.userLogin = async (req, res, next) => {
    ...
    try {
        ...
        const token = jwt.sign({
            email: loadedUser.email, userId: loadedUser._id.toString(), name: loadedUser.name, // password: password,
            role: 'user',
        ...
        res
            ...
            .status(200)
            .json({
                token: token, userId: loadedUser._id.toString(), name: loadedUser.name, email: email, role: 'user',
            });
    } catch (err) {
        if (!err.statusCode) {
            err.statusCode = 500;
...
exports.adminLogin = async (req, res, next) => {
    ...
    try {
        ...
        res
            ...
            .status(200).json({
            token: token, userId: loadedAdmin._id.toString(), name: loadedAdmin.name, email: email, role: 'admin',
        });
    } catch (err) {
        if (!err.statusCode) {
            err.statusCode = 500;
